...
@section('content')
    ...
    <div id="newsManagementContent" class="opacity-0 animate-[fadeIn_0.5s_ease-in-out_forwards]">
        <div class="bg-white rounded-xl shadow-sm p-4 md:p-6 mb-6">
            <div class="flex flex-col md:flex-row md:items-center justify-between mb-6">
                <h2 class="text-lg font-bold text-gray-800 mb-4 md:mb-0">Kelola Guru dan Staff</h2>
                <div class="flex flex-col sm:flex-row space-y-3 sm:space-y-0 sm:space-x-3">
                    <div class="relative">
                        <input type="text" id="customSearch" placeholder="Cari guru atau staff..."
                    ...
                    <button id="addNewsButton" onclick="location.href='{{ route('lecture-staff.create') }}'"
                        ...
                        <span>Tambah Guru atau Staff</span>
                    </button>
                </div>
            </div>

            <div class="overflow-x-auto">
                <table id="lectureStaffTable" class="w-full">
                    <thead>
                        <tr class="text-left text-gray-600 text-sm border-b border-gray-200">
                            ...
                            <th class="pb-3 font-medium hidden md:table-cell">Posisi</th>
    ...
    @include('admin.lecture-staff.modal')
@endsection
...
@section('custom-js')
    <script>
        ...
        const dataTable = $('#lectureStaffTable').DataTable({
            dom: 'rt<"flex items-center justify-between mt-4"lip>', // Custom dom
            processing: true,
            serverSide: true,
            ajax: "{{ route('lecture-staff.all') }}",
            columns: [{
                    data: 'checkbox',
                    orderable: false,
                    searchable: false
            ...
            language: {
                lengthMenu: "_MENU_",
                search: "",
                searchPlaceholder: "Cari Guru dan Staff..."
            }
        });
        $('#customSearch').on('keyup', function() {
            $('#lectureStaffTable').DataTable().search(this.value).draw();
        });

        $('#lectureStaffTable').on('click', '.delete-lecture-staff', function() {
            ...
            if (row) {
                ...
                $('#delete-lecture-staff-name').text(row.name);
                ...
                $('#confirm-delete').off('click').on('click', function() {
                    ...
                    $.ajax({
                        url: `/admin/lecture-staff/destroy/${row.id}`,
                        type: 'POST',
                        data: formData,
                        processData: false,
                        contentType: false,
                        success: function() {
                            ...
                            showNotification('Guru atau Staff berhasil dihapus!');
                            $('#delete-modal').addClass('hidden');
                        },
                        error: function(xhr) {
                            let message = 'Terjadi kesalahan saat menghapus guru atau staff.';
